...
2. 使用 Skill: 按照用户目标和需要用到的命令行工具, 优先匹配并调用符合要求的 Skills. **注意**: 如果语境涉及到**安装**任何东西, 则无论安装指南如何, 必须**立刻**引入软件包安装 Skill!
3. 执行计划: 简要列出你在本次对话中要执行的工具调用和输出的内容大纲.
   1. 如果需要询问用户问题, 必须通过 question 工具与用户交互.
   1. 如果要执行底层终端命令, 统一通过 bash 工具执行.
